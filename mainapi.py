...
def add_badge(badge: Badge, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    new_user = User(uid=badge.uid, prenom=badge.name, role=badge.role)
    db.add(new_user)
    db.commit()
    return {"message": "Badge ajouté", "uid": badge.uid, "prenom": badge.name, "role": badge.role}
...
def scan_badge(badge_uid: str, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if badge:
        return {"message": "Badge reconnu", "id": badge.uid, "role": badge.role, "prenom": badge.name}
...
def update_badge(badge_uid: str, badge: Badge, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    user_db.prenom = badge.name
    user_db.role = badge.role
    db.commit()
    return {"message": "Badge mis à jour", "uid": badge_uid, "prenom": badge.name, "role": badge.role}
